...
DB_PATH = "persistent_sessions.db"
...
def save_user_session(user_id, samples_list, chat_history):
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        ...
        cursor.execute("""
            INSERT INTO user_sessions (user_id, samples_data, chat_history, last_updated)
            VALUES (?, ?, ?, CURRENT_TIMESTAMP)
            ON CONFLICT(user_id) DO UPDATE SET
                samples_data = excluded.samples_data,
                chat_history = excluded.chat_history,
                last_updated = CURRENT_TIMESTAMP
        """, (user_id, samples_json, chat_json))
        conn.commit()
        conn.close()
    except Exception as e:
...
def load_user_session(user_id):
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT samples_data, chat_history FROM user_sessions WHERE user_id = ?", (user_id,))
        row = cursor.fetchone()
        conn.close()
        if row:
...
if "session_loaded" not in st.session_state:
    saved_samples, saved_chat = load_user_session(user_id)
    if saved_samples is not None:
        st.session_state.samples_list = saved_samples
