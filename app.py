...
conn.commit()
conn.close()
# Resolve user ID (identifies the browser)
...
"""
            ...
            if any(term in error_msg.lower() for term in ["429", "503", "quota", "rate", "unavailable"]):
                if i < len(GEMINI_API_KEYS) - 1:
                    print(f"Key {i+1} failed/busy. Trying backup key...")
                    continue  # Try next key
                return "All API keys are currently busy or rate-limited. Please try again in a few seconds.", None
...
# -----------------------------------------------
# AUTO-SAVE LOGIC (Paste at the very bottom of app.py)
# -----------------------------------------------
def auto_save_chat():
    try:
