...
# Add at the bottom of calculations.py

def validate_rheology(theta600: float, theta300: float) -> str | None:
    if theta600 == 0 and theta300 == 0:
        return None  # No reading entered yet, no warning needed
...
def validate_yp(yp: float) -> str | None:
    if yp < 0:
        return "Warning: YP cannot be negative. Ensure that PV is positive and also less than or equal to θ300. Check dial readings."
    return None

def validate_bf(bf: float) -> str | None:
    if bf < 0:
        return "Warning: BF cannot be negative. Check the mud weight input (safe limit using barite as weighting material: 22 ppg or below)."
    return None